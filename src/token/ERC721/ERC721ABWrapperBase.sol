...
contract ERC721ABWrapperBase is ERC721Upgradeable, AccessControlUpgradeable {
     ...
     */
    function initDrop(string calldata _baseUri) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function initRoyalty(address _publisher, address _royaltyCurrency) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256, uint256) internal override(ERC721Upgradeable) {
        if (address(abRoyalty) == address(0)) {
            if (_to == address(this)) {
                ...
                abHolderRegistry.registerHolderChange721(_from, publisher, dropId, 1);
            } else if (_from == address(this)) {
                // Redirect royalty from the publisher of this collection
                abHolderRegistry.registerHolderChange721(publisher, _to, dropId, 1);
            } else {
                abHolderRegistry.registerHolderChange721(_from, _to, dropId, 1);
            }
        } else {
            if (_to == address(this)) {
                // Redirect royalty to the publisher of this collection
